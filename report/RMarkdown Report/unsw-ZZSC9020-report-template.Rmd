...
knitr::opts_chunk$set(echo = TRUE, warning = FALSE, fig.height = 6, fig.width = 4)
library(reticulate)
...
#python_packages <- c("matplotlib", "numpy", "pandas")

#if (!(python_packages %in% py_list_packages(type = "conda")[['packages']])) {
#  py_install(packages = python_packages)
#}
...
df_temperature_temp14d = df_temperature.copy()

df_temperature_temp14d['temp_14d'] = df_temperature_temp14d.temperature.rolling(window=672).mean()  # 14 days for hourly data

plt.figure(figsize=(6, 4))
plt.plot(df_temperature_temp14d['date_time'], df_temperature_temp14d['temp_14d'], 
    label='14-Day Rolling Avg', color='red')
plt.title('Smoothed Temperature Over Time')
plt.xlabel('Date')
...
\noindent Extreme temperature events occur consistently across the year, with a lower count (but still significant) in April and November. Extreme heat days are defined as days with temperatures above the 90th percentile in the past year, and extreme cold days are defined as days with temperatures below the 10th percentile in the past year.

```{python, echo = FALSE}

df_temperature_extreme = df_temperature.copy()

### Classify extreme weather for each row
df_temperature_extreme['extreme_type'] = np.select(
    [
        df_temperature_extreme['temperature'] > df_temperature_extreme.temperature.rolling(20030).quantile(0.9),
        df_temperature_extreme['temperature'] < df_temperature_extreme.temperature.rolling(20030).quantile(0.1)
    ],
...
### Aggregate by date
def categorise_day(group_day):
    if 'Extreme Heat' in group_day.values:
        return 'Extreme Heat'
    elif 'Extreme Cold' in group_day.values:
        return 'Extreme Cold'
    else:
        return 'Normal'

df_temperature_extreme['date'] = df_temperature_extreme['date_time'].dt.date
df_extreme_daily = df_temperature_extreme.groupby(by = 'date', as_index = False).agg({'extreme_type': categorise_day})
df_extreme_daily.date = pd.to_datetime(
    df_extreme_daily.date, format = "%Y-%m-%d")

### Count extreme weather days
extreme_counts = df_extreme_daily[df_extreme_daily['extreme_type'] != 'Normal'].groupby([df_extreme_daily['date'].dt.month, 'extreme_type']).size().unstack()

### Plot
extreme_counts.plot(kind='bar', stacked=True, figsize=(6,4), colormap='coolwarm')
plt.title('Extreme Temperature Days per Month')
plt.xlabel('Month')
plt.ylabel('Count')
plt.legend()
plt.grid(True)
...
df_demand_14d = df_demand.copy()

df_demand_14d['dem_14d'] = df_demand.total_demand.rolling(window=672).mean()  # 14 days for hourly data

plt.figure(figsize=(6, 4))
plt.plot(df_demand_14d['date_time'], df_demand_14d['dem_14d'],
    label='14-Day Rolling Avg', color='blue')
plt.title('Smoothed Total Demand Over Time')
plt.xlabel('Year')
...
\noindent Demand is stable during the weekdays and declines consistently at the weekends. This may be due to many businesses closing during weekends. 


```{python, echo = FALSE}

df_demand_weekday = df_demand.copy()

df_demand_weekday['weekday'] = df_demand.date_time.dt.day_of_week
df_demand_weekday['weekday_name'] = df_demand.date_time.dt.day_name()

plt.figure(figsize = (6,4))
sns.boxplot(data = df_demand_weekday.groupby("date_time", as_index = False).first().sort_values("weekday"), x = 'weekday_name', y = "total_demand", hue = 'weekday', palette = 'Blues', showfliers = False, legend = False)
plt.grid(alpha = 0.5)
...
df_demand_month = df_demand.copy()

df_demand_month['month'] = df_demand.date_time.dt.month
df_demand_month['month_name'] = df_demand.date_time.dt.month_name().str[:3]

plt.figure(figsize = (6,4))
sns.boxplot(data = df_demand_month.groupby("date_time", as_index = False).first().sort_values("month"), x = 'month_name', y = "total_demand", hue = 'month', palette = 'Blues', showfliers = False, legend = False)
plt.grid(alpha = 0.5)
...
In this next section, we will examine relationships between the datasets to find if and how temperature affects both the electricity demand and its forecast. To proceed with this analysis, all the datasets are merged following the data cleaning processes mentioned in Section x.x.
...
plt.figure(figsize=(6, 4))
plt.scatter(df_sampled.temperature, df_sampled.total_demand, alpha=0.5, s=20, label='Data Points')
plt.plot(x_trend, y_trend, color='red', linewidth=2, label='Linear Trendline')

plt.xlabel('Temperature')
plt.ylabel('Actual Electricity Demand')
...
\noindent The same pattern emerges when plotting temperature against the forecasted electricity demand.
...
- Forecasting models capture seasonal trends:
Forecasted electricity demand shows a similar U-shaped relationship with temperature, indicating that the models are aligned with seasonal usage patterns.
